...
const searchQuery = encodeURIComponent('cat:physics.med-ph OR cat:q-bio.QM');
...
async function fetchPapers() {
    try {
        ...
        const papers = result.feed.entry.map(entry => ({
            title: entry.title[0],
            authors: entry.author ? entry.author.map(author => author.name[0]).join(', ') : 'No authors listed',
